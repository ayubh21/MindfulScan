...
with open('dataset_1.csv', 'r') as csv_file:
    ...
    with open('new_dataset.csv', 'w', newline='') as new_file:
        ...
        for line in csv_reader:
            if line[1] != "":
                count = int(line[1])
                hateSpeech = int(line[2])
                offensiveLanguage = int(line[3])
                neither = int(line[4])
            
                biggestCount = max([hateSpeech, offensiveLanguage, neither])
                

                counts = [
                    {"name": "hate", "count": hateSpeech},
                    {"name": "offensive", "count": offensiveLanguage},
                    {"name": "neither", "count": neither},
                ]
